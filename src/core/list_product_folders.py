...
# Returns the images sitting directly inside one product folder
def list_images_in_folder(service, folder_id):
    ...
    return image_results.get('files', [])
...
def list_product_folders(service, parent_folder_id):
    ...
    for product_folder in product_folders:
        images = list_images_in_folder(service, product_folder['id'])
        print("Images found in", product_folder['name'], ":", len(images))

        folders.append({
            'id': product_folder['id'],
